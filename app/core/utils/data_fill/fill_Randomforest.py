...
def random_forest(df,var_type_dict):
    ...
    null_columns_num = df.isnull().sum(axis=0)[df.isnull().sum(axis=0) > 0]#得到缺失值的列名
    ...
    for miss_column in null_columns_num.sort_values().index:
        df_nonempty = df_new.dropna(axis=1, how='any', inplace=False)# 删除含有空值的列.
        ...
        Xtrain = df_nonempty.iloc[ytrain.index, :] # 新特征矩阵上，被选出来要填充的特征的非空值对应的样本
        ...
        for i, row_idx in enumerate(Xtest.index):
            df_new.loc[row_idx:row_idx, miss_column].fillna(filled_value[i], inplace=True)
